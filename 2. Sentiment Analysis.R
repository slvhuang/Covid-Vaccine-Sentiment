...
wordcloud(tweets_cases_vaccine$text, min.freq = 700, colors = brewer.pal(8, "Dark2"),
...
ggplot(before_vac, aes(x = factor(user_state), y = syuzhet)) + geom_boxplot()
senti_state_before <- aggregate(before_vac$syuzhet, list(before_vac$user_state), mean)

# Sentiment by state after vaccination
ggplot(after_vac, aes(x = factor(user_state), y = syuzhet)) + geom_boxplot()
senti_state_after <- aggregate(after_vac$syuzhet, list(after_vac$user_state), mean)
...
ggplot(before_vac, aes(x = date, y = syuzhet, color = user_state)) + geom_point()
ggplot(after_vac, aes(x = Admin_Per_100K, y = syuzhet)) + geom_point()
